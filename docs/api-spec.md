...
> - **숫자 레벨(`LV.n`)·견적 표시는 넣지 않는다** (v8이 조건부 허용했다가 v8.1에서 재철회, 계속 유지).
> - **⚠️ v8.2 반영 필요 (2026-09-11, 서경대 해커톤 멘토링 반영)**: **정성적 등급(긴급/주의/권장) 표시는 재도입됐다** — v8.1에서 등급 전체를 철회했었으나, 고지 문구를 동반하는 조건으로 다시 허용. 계산 API(`diagnoses/calculate`) 응답 구조 자체는 안 바뀐다 — 등급은 프론트가 기존 `baseline`/`scenarios` 값을 보고 표시 여부를 판단하는 것이지, 서버가 별도 필드로 내려주는 것이 아니다(산정 임계값이 아직 정책 확정 전이라 서버 필드로 만들면 임의 수치를 구현에 박아넣게 됨 — db-spec.md 8장 참고). 타겟층도 소유자+세입자로 확대됐으나 API 계약에는 영향 없음(화면 문구만 조정).
...
**구현 완료 (2026-09-11)**: `app/api/routes/bills.py`. 명세대로 요청 형태만 검증하고 실제 비교 계산은 아직 하지 않는다 — 항상 `comparison_result: null` + 고정 고지 문구를 반환한다. 비교 정책이 확정되면 그때 실제 로직을 채운다.
