...
## 0.3 향후 확장 메모 (MVP 범위 아님, 설계 반영 안 함)

- "회원이 과거 진단 기록을 조회할 수 있게" 하는 기능이 향후 검토될 수 있다는 의견이 있었음. 다만 아직 확정된 요구사항이 아니고, 이 기능을 하려면 계정/로그인이 선행돼야 하는데 계정/로그인은 PRD 3.2 "제외 대상"에 명시돼 있어 이번 MVP 범위에 포함하지 않는다.
- 지금 이 문서의 API/DTO는 이 가능성 때문에 별도로 설계를 바꾸지 않는다. 나중에 실제로 이 기능을 하기로 결정되면, 그때 계정 체계 + 진단 이력 저장 스키마를 새로 설계해야 한다(현재 무상태 설계와는 별개의 큰 변경).
...
| `image` | file | Y | jpeg/png/webp, 최대 용량은 정책 확정 필요(제안: 10MB) |
